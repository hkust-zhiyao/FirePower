...
def res_augment(res_model,target_feature,target_label,valid_index):
    ...
    for component in event_feature_of_components.keys():
        ...
        if confidence > 0.96: # and component!="DCacheTagArray" and component!="DCacheDataArray":
            print(component,config_name[best_feature])
...
def draw_figure(gt,pd,name):
    ...
    color_set = ['b','g','r','c','m','y','k','skyblue','olive','gray','coral','gold','peru','pink','cyan','red']
...
def known_n_config(known,target_uarch,source_uarch):
    ...
    res_model, res_param = res_augment(res_model,target_feature.reshape(target_feature.shape[0]//8,8,target_feature.shape[1])[:,0,:],np.average(target_label.reshape(target_label.shape[0]//8,8,target_label.shape[1]),axis=1),training_config)
    ...
    for i in range(23):
        draw_figure(target_label[testing_set,i],prediction[:,i],"McPATransfer_{}_".format(known)+figure_name[i]+"_{}_from_{}".format(target_uarch,source_uarch))
    return
...
np.save("0_mapctransfer_r",np.array(r_list))
np.save("0_mapctransfer_mape",np.array(mape_list))