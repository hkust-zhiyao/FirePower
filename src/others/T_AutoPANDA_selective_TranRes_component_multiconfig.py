...
def res_augment(res_model,target_feature,target_label,valid_index):
    ...
    pseudo_label_list = res_per_comp(res_model,target_feature)
    ...
    for component in event_feature_of_components.keys():
        feature_index = params_feature_of_components[component]
        feature = target_feature[:,feature_index]
        
        label_index = iter + 1
        label = calibrated_pseudo_label[:,label_index]
        
        model = xgb.XGBRegressor(n_estimators=100,max_depth=3)
        res_model_this_component = model.fit(feature,label)

        print(feature.shape,label.shape)

        if component == "BPTAGE" or component == "BPBTB" or component == "BPOther":
...
def draw_figure(gt,pd,name):
    ...
    plt.savefig("AutoPANDA_Sel_TranRes_multiconfig/{}.jpg".format(name),dpi=200)
    plt.close()
    return
...
def known_n_config(known,target_uarch,source_uarch):
    ...
    res_model = res_augment(res_model,target_feature.reshape(target_feature.shape[0]//8,8,target_feature.shape[1])[:,0,:],np.average(target_label.reshape(target_label.shape[0]//8,8,target_label.shape[1]),axis=1),training_config)
