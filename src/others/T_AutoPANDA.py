...
def unknown_n_config(unknown,target_uarch,source_uarch):
    ...
    for i in range(fold):
        ...
        res_model = get_resource_function(target_feature[training_set], target_label[training_set])
...
np.save("T_XS_mape.npy",np.array(curve_mape))
np.save("T_XS_r.npy",np.array(curve_r))
...
np.save("T_BOOM_mape.npy",np.array(curve_mape))
np.save("T_BOOM_r.npy",np.array(curve_r))