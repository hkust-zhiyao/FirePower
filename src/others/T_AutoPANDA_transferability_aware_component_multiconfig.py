...
def build_model(feature,label,res_model,res_params):
    model_list = []
    iter = 0
    for component in event_feature_of_components.keys():
        ...
        res_index = res_params[component]
        component_feature = feature[:,feature_index]
        res_feature = feature[:,res_index]
        label_index = iter + 1
...
def predict_per_comp(res_model,res_params,model_list,test_feature):
    iter = 0
    power_list = []
    for component in event_feature_of_components.keys():
        # get model
        ...
        res_feature = test_feature[:,res_index]
...
def predict(model_list,res_model,res_params,test_feature):
    power_list = predict_per_comp(res_model,res_params,model_list,test_feature)
...
def draw_res(gt,pd,name):
    ...
    #     x = gt[i*8:(i+1)*8]
    #     y = pd[i*8:(i+1)*8]
    plt.scatter(gt,pd,marker='.',color=color_set[1],alpha=0.5,s=160)
    ...
    plt.ylabel(r"${F_{hw}}$"+' Prediction (W)', fontsize=22)
    if name.find("Dtlb")!=-1:
        plt.xticks([0,0.01,0.02,0.03],fontsize=20)
        plt.yticks([0,0.01,0.02,0.03],fontsize=20)
        plt.plot([0,0.03],[0,0.03],color='silver')
        max_value=0.03
    elif name.find("LSU")!=-1:
        plt.xticks([0,0.02,0.04,0.06,0.08],fontsize=20)
        plt.yticks([0,0.02,0.04,0.06,0.08],fontsize=20)
        plt.plot([0,0.08],[0,0.08],color='silver')
        max_value=0.08
    elif name.find("OtherLogic")!=-1:
        plt.xticks([0,0.01,0.02,0.03,0.04],fontsize=20)
        plt.yticks([0,0.01,0.02,0.03,0.04],fontsize=20)
        plt.plot([0,0.04],[0,0.04],color='silver')
        max_value=0.04
    elif name.find("DCacheDataArray")!=-1:
        plt.xticks([0,0.04,0.08,0.12,0.16],fontsize=20)
        plt.yticks([0,0.04,0.08,0.12,0.16],fontsize=20)
        plt.plot([0,0.16],[0,0.16],color='silver')
        max_value=0.16
    else:
    ...
    plt.text(0,max_value/15*14,"MAPE={:.2f}%".format(mape_report*100),fontsize=20,bbox=dict(boxstyle='round,pad=0.5', fc='white', ec='silver',lw=5 ,alpha=0.7))
    ax = plt.gca()
    for axis in ['top','bottom','left','right']:
        ax.spines[axis].set_linewidth(2.5)
    plt.savefig("res_validation/{}.jpg".format(name),dpi=200)
    plt.close()
    return

def res_augment(res_model,target_feature,target_label,valid_index,target_uarch,source_uarch):

    importance_rank = []
    for model in res_model:
        importance_rank.append(np.argsort(-model.feature_importances_))


    result_model = []
    iter = 0
    res_params_feature_of_components = {}
    for component in event_feature_of_components.keys():
        feature_index = params_feature_of_components[component]
        # importance_acc = 0
        # prune_feature_index = []
        # for idx in range(len(feature_index)):
        #     prune_feature_index.append(feature_index[importance_rank[iter][idx]])
        #     importance_acc = importance_acc + res_model[iter].feature_importances_[importance_rank[iter][idx]]
        #     print(res_model[iter].feature_importances_[importance_rank[iter][idx]])
        #     if importance_acc>1:
        #         break

        check_feature = target_feature[valid_index]
        check_label = target_label[valid_index]
        res_pred_train = res_model[iter].predict(check_feature[:,feature_index])
        res_label_train = check_label[:,iter+1]
        res_pred = res_model[iter].predict(target_feature[:,feature_index])
        res_label = target_label[:,iter+1]
        draw_res(res_label_train,res_pred_train,"Validation_train_{}_{}_from_{}".format(component,target_uarch,source_uarch))
        draw_res(res_label,res_pred,"Validation_all_{}_{}_from_{}".format(component,target_uarch,source_uarch))


        print(component,res_model[iter].feature_importances_)
        confidence = res_model[iter].feature_importances_[importance_rank[iter][0]]
        best_feature = feature_index[importance_rank[iter][0]]

        if confidence > 0.96 and component!="DCacheTagArray" and component!="DCacheDataArray":
            model = LinearRegression()
            feature = target_feature[:,[best_feature]]
            label_index = iter + 1
            label = target_label[:,label_index]
            res_model_this_component = model.fit(feature[valid_index],label[valid_index])
            res_params_feature_of_components[component] = [best_feature]
        else:
            res_model_this_component = res_model[iter]
            res_params_feature_of_components[component] = params_feature_of_components[component]

        result_model.append(res_model_this_component)
        iter = iter + 1
    return result_model, res_params_feature_of_components
...
def draw_figure(gt,pd,name):
    ...
    for axis in ['top','bottom','left','right']:
        ax.spines[axis].set_linewidth(2.5)
    plt.savefig("AutoPANDA_importance_multiconfig/{}.jpg".format(name),dpi=200)
    plt.close()
    return
...
def known_n_config(known,target_uarch,source_uarch):
    ...
    print(source_feature[:,7])
    print(target_feature[:,7])
    ...
    res_model, res_param = res_augment(res_model,target_feature.reshape(target_feature.shape[0]//8,8,target_feature.shape[1])[:,0,:],np.average(target_label.reshape(target_label.shape[0]//8,8,target_label.shape[1]),axis=1),training_config,target_uarch,source_uarch)
    print(params_feature_of_components)
    print(res_param)
    target_model = build_model(target_feature[training_set], target_label[training_set], res_model, res_param)
    prediction = predict(target_model, res_model, res_param, target_feature[testing_set])

    for i in range(23):
        draw_figure(target_label[testing_set,i],prediction[:,i],figure_name[i]+"_{}_from_{}".format(target_uarch,source_uarch))
    return
