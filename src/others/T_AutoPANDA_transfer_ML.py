...
def test_for_one_component_ml(res_model, model, res_feat, test_feat):
    pred_part = model.predict(test_feat)
    return pred_part
...
def predict_per_comp_ml(res_model,model_list,test_feature):
    iter = 0
    power_list = []
    for component in event_feature_of_components.keys():
        # get model
        ...
        # compute and accumulate power
        power_component = test_for_one_component_ml(res_model[iter],model_component,res_feature,component_feature)
        power_list.append(power_component)
...
def get_ml_part(gt_feature,gt_label,res_model):
    iter = 0
    ml_label = np.zeros(gt_label.shape)
    for component in event_feature_of_components.keys():
        ...
        end_event = event_feature_of_components[component][1]
        # feature_index = params_feature_of_components[component] + [item for item in range(start_event,end_event)]
        # component_feature = gt_feature[:,feature_index]
        res_feature = gt_feature[:,params_feature_of_components[component]]
        label_index = iter + 1
        # component_label = gt_label[:,label_index]
        ml_label[:,label_index] = gt_label[:,label_index] / res_model[iter].predict(res_feature)
...
def data_augment(source_model,target_feature,target_label,valid_index,res_model):
    # ml part prediction
    pseudo_label_list = predict_per_comp_ml(res_model,source_model,target_feature)
    
    # extract ml part label
    gt_feature = target_feature[valid_index]
    gt_label = target_label[valid_index]
    gt_label = get_ml_part(gt_feature,gt_label,res_model)
    
    # calibrate ml pseudo label
    ...
    iter = 0
    for component in event_feature_of_components.keys():
        res_feature = target_feature[:,params_feature_of_components[component]]
        #print(res_feature.shape,calibrated_pseudo_label.shape)
        label_index = iter + 1
        calibrated_pseudo_label[:,label_index] = calibrated_pseudo_label[:,label_index] * res_model[iter].predict(res_feature)
        iter = iter + 1
    calibrated_pseudo_label[sample_idx,0] = np.sum(calibrated_pseudo_label[sample_idx,1:])
