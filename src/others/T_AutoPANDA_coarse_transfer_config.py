...
def load_data(uarch):
    ...
    print(label.shape)
    label_tmp = np.zeros((label.shape[0],14)) 
    label_tmp[:,:10] = label[:,:10]
    label_tmp[:,10] = np.sum(label[:,10:13],axis=1)
    label_tmp[:,11] = np.sum(label[:,13:17],axis=1)
    label_tmp[:,12] = np.sum(label[:,17:20],axis=1)
    label_tmp[:,13] = np.sum(label[:,20:23],axis=1)
    label = label_tmp
...
np.save("npy/T_coarse_XS_from_BOOM_mape.npy",np.array(curve_mape))
np.save("npy/T_coarse_XS_from_BOOM_r.npy",np.array(curve_r))
...
np.save("npy/T_coarse_BOOM_from_XS_mape.npy",np.array(curve_mape))
np.save("npy/T_coarse_BOOM_from_XS_r.npy",np.array(curve_r))