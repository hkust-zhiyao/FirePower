...
def draw_figure(gt,pd,name):
    ...
    plt.savefig("McPAT-Calib_multiconfig/{}.jpg".format(name),dpi=200)
    plt.close()
    return
...
def known_n_config(known,target_uarch,source_uarch):
    ...
    target_model = build_model(target_feature[training_set],target_label[training_set,0])
    ...
    draw_figure(label[testing_set],prediction,"McPAT-Calib_{}_{}_from_{}".format(known,target_uarch,source_uarch))
...
np.save("0_mapct_calib_r",np.array(r_list))
np.save("0_mapct_calib_mape",np.array(mape_list))