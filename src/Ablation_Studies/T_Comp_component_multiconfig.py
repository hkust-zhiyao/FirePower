...
def draw_figure(gt,pd,name):
    ...
    plt.savefig("Comp_multiconfig/{}.jpg".format(name),dpi=200)
    plt.close()
    return
...
def known_n_config(known,target_uarch,source_uarch):
    ...
    for i in range(23):
        draw_figure(target_label[testing_set,i],prediction[:,i],"Comp_{}_".format(known)+figure_name[i]+"_{}_from_{}".format(target_uarch,source_uarch))
    return
...
np.save("2_comp_r",np.array(r_list))
np.save("2_comp_mape",np.array(mape_list))